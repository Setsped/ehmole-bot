...
module.exports = {
	...
	callback: async (message) => {
		...
		let posicao = player.position, tempo, TrackDuration;
		(tempo = msToHour(posicao)), (posicao = ~~(posicao / 1000));

		if (current.duration >= 9023372036854776000) {
			TrackDuration = 'Live';
		}
		else {
			TrackDuration = ssTohms(current.duration / 1000);
		}

		embed
			.setDescription(`[${current.title}](${current.uri || null})`)
			.addFields(
				{
					name: '\u200B',
					value: '`' + `${tempo}[${progressBarEnhanced(posicao, current.duration / 1000, 15)}]${TrackDuration}` + '`',
				},
				{
					name: '\u200B',
					value: '`Requested by: `' + `${current.requester.username}`,
				},
			)
