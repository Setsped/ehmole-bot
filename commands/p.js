...
module.exports = {
	...
	callback: async (message, arguments, manager, client) => {
		...
		if (arguments.join().includes('&ab_channel')) {
			const argumentArr = arguments.join().split('&ab_channel');
		...
		try {
			...
			return message.reply(
				`Ocorreu um Erro tentando pesquisar essa musica: ${err.message}`,
			);
		}
		...
		case 'TRACK_LOADED':
			if (player.playing) {
				...
				if (res.tracks[0].duration >= 90233720368547760) {
					TrackDuration = 'Live';
				}
				else {
					TrackDuration = ssTohms(res.tracks[0].duration / 1000);
				}
				...
				const embedAF = new Discord.MessageEmbed()
					...
					.setColor('#faebd7')
					.setThumbnail(
						`${res.tracks[0].thumbnail || SpotifyThumb}`,
					)
					.setDescription(
						`[${res.tracks[0].title}](${
					...
					.addFields(
						...
						{
							name: '\u200B',
							value:'`Requested by: `' + '<@' + res.tracks[0].requester + '>',
						},
					);
		...
		case 'PLAYLIST_LOADED':
			...
			return message.reply(
				`Playlist adicionada \`${res.playlist.name}\` com  ${res.tracks.length} musicas.`,
			);
