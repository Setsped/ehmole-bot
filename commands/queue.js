...
module.exports = {
    ...
    callback: async (message, arguments, text,client) => {
        ...
        var songQueueArray = [], page = arguments[0] || 1;
        var disab = [false, false, false, false];
        let embedMsg;

        if (bot_channel && songQueue?.length) {
            for (let i = 0; songQueue.length > i; i++) {
                songQueueArray.push(`${i + 1}. [`+ssTohms(songQueue[i].duration/1000)+`]` +
                    ` [${songQueue[i].title}](${songQueue[i].uri || null})`
        ...
        async function send() {
            if (page === songQueueArray.length) { disab[2] = true; disab[3] = true } else { disab[2] = false; disab[3] = false }

            if (page === 1) { disab[0] = true; disab[1] = true } else { disab[0] = false; disab[1] = false }

            embed.setDescription(songQueueArray[page - 1])
            embed.setFooter("página " + page + " de " + songQueueArray.length)

            let button1 = new botao.MessageButton()
                .setID('firstPage')
                .setEmoji('⏮')
                .setDisabled(disab[0])
                .setStyle(1);
            ...
            let button2 = new botao.MessageButton()
                .setID('previousPage')
                .setDisabled(disab[1])
                .setEmoji('⏪')
                .setStyle(1);

            let button3 = new botao.MessageButton()
                .setID('nextPage')
                .setEmoji('⏩')
                .setDisabled(disab[2])
                .setStyle(1);

            let button4 = new botao.MessageButton()
                .setID('lastPage')
                .setEmoji('⏭')
                .setDisabled(disab[3])
                .setStyle(1);
            ...
            if (embedMsg){
                ...
                embedMsg = await message.channel.send({ content: embed, components: [row] })
            }

            const filter = (button) => button.clicker.user.id === message.author.id
            const collector = embedMsg.createButtonCollector(filter, { max: 1, time: 30000 })
            ...
            collector.on('collect', async (button) => {
                ...
                if (button.id === "firstPage") {
                    ...
                    .setFooter("page " + page + " de " + songQueueArray.length)
                    send()
                }
                ...
                if (button.id === "lastPage") {
                    ...
                    embed
                    .setDescription(songQueueArray[page - 1])
                    .setFooter("página " + page + " de " + songQueueArray.length)
                    send()
                }
