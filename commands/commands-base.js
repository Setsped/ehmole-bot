...
module.exports.listen = (client) =>{
   client.on("message", (message) => {
    ...
    if (message.content == '*ban <@!804422431548112896>' || message.content == '*kick <@!804422431548112896>') return message.channel.send("Ta loco man?? 🤨");
      ...
      const {
        ...
        for (const requiredRole of requiredRoles) {
          ...
          if (!role || !member.roles.cache.has(role.id)) {
              message.reply(`Você precisa ter a "${requiredRole}" role para usar esse comando`);
              return;
          }
        }
        ...
        if (arguments.length < minArgs || 
          (maxArgs !== null && arguments.length > maxArgs)) { // Setei alias em cima pra ser alguma coisa do commands
            ...
            message.reply(`Syntax incorreta burro. Tem que ser ${name} ${expectedArgs}`);
            return;
        }
